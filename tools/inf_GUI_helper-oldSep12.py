...
class myTab:
    ...
    def add_props(self):
        ...
        ttk.Label(self.parent, text='Custom EOS').grid(column=4, row=self.row, sticky='NW')
        self.row += 1
        ...
        self.increment = StringVar(); self.increment.set('Default')
        ttk.Entry(self.parent, textvariable=self.increment, width=7).grid(column=3, row=self.row, sticky='NW')
        ttk.Label(self.parent, text='Custom Increment').grid(column=4, row=self.row, sticky='NW')
        self.row += 1
        
        self.increment_type = StringVar()
        ttk.Radiobutton(self.parent, text='Use fast increment', value='fast',
                        variable=self.increment_type).grid(row=self.row, column=2, sticky='NW')
        ttk.Radiobutton(self.parent, text='Use good increment', value='good',
                        variable=self.increment_type).grid(row=self.row, column=4, sticky='NW')
        self.row += 1
...
#            else:
        ...
        self.inf['PARM'] = ['pparray r u acc rho te ti tr pres zbar',
                            'parm nstop 5000000',
                            'parm IRDTRN 0',
    ...
    def calc_increments(self, layers, FZM_match_density=False):
        ...
        increment_range = np.arange(0.990, 1.010, step=0.0001)
        ###
