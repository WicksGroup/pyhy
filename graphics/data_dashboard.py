    ...
    if list_of_contents is not None:
        ...
        try:
            ...
            data = {'X': hyades.x[0, :].tolist(),
                    ...
                    'var': var,
                    'material properties': hyades.layers,
                    'filename': basename
                    }
...
def update_lineout(selected_time, children):
    ...
    for mat in material_properties:
        ...
        traces.append(go.Scatter(x=X[start:stop], y=output[start:stop, index],
                                 name=mat['Name'],
                                 line=dict(width=4),
                                 )
                      )
        dashedX = [mat['X Stop']] * 2
        dashedY = [0, output.max() + 1]
        traces.append(go.Scatter(x=dashedX, y=dashedY,
                                 showlegend=False,
                                 line=dict(color='black', width=1, dash='dot')
                                 )
                      )
        annotations.append(go.layout.Annotation(x=(mat['X Start'] + mat['X Stop']) / 2,
                                                y=output.max() * 0.9,
                                                # xref="x",
                                                # yref="y",
                                                text=mat['Name'],
